...
ec2 = gl.deploy.ec2_cluster.create(name='carwashr-4', s3_path=s3_path, ec2_config=gl.deploy.Ec2Config())
...
def train_carwash_network():
    ...
    sf["label"] = sf.apply(
        lambda row: int(Counter(row["labels"]).most_common()[0][0]))  # use mode label as authoritative label
    train, test = sf.random_split(0.8)
    feature_extractor = gl.feature_engineering.create(sf,
                                                      DeepFeatureExtractor(feature="image",
